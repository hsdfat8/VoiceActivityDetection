...
Word16 vad2 (Word16 * farray_ptr, vadState2 * st)
{
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++)
	{
		...
		/*   ch_snr = bround((snr/(3/8))>>8)                          */
		/*          = bround(((0.6667*snr)<<2)>>8)                    */
		/*          = bround((0.6667*snr)>>6)                         */
	...
	else
	{
		...
		if (sub(xt, st->tsnr) > 0)
		{
			...
			st->tsnr = bround(L_add(L_mult(29491, st->tsnr), L_mult(3277, xt)));
		}
		/* else if (xt > 0.625*tsnr) */	
		else if (sub(xt, mult(20480, st->tsnr)) > 0)
		{
			/* tsnr = 0.998*tsnr + 0.002*xt; */
			st->tsnr = bround(L_add(L_mult(32702, st->tsnr), L_mult(66, xt)));
		}
	}
	...
	if (xt < 0)
	{
		...
		tmp = bround(L_shl(L_mult(xt, xt), 7));
		st->negSNRvar = bround(L_add(L_mult(32440, st->negSNRvar), L_mult(328, tmp)));
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++)
	{
		...
		st->ch_enrg_long_db[i] = bround(L_add(Ltmp1, Ltmp2));
	}
